...
public class CaptionCorrection {
	...
	public boolean meetingAdmin(String adminPwd) {
		...
		if (adminPwd != null) {
				if (adminPwd.equals("password123456")) {
				//check in database
				result = true;
			}
		}
	...
	//open the room back up
		...
		if ((mr == null) || (!mr.roomExists()) ){
			...
			writeToAdmin(request, response, "Meeting room \"" + roomName + "\" does not exist.  Choose a different room or \"" + ADMIN_COMMAND_CREATE_PARAM + "\" instead of \"" + ADMIN_COMMAND_OPEN_PARAM + "\".");
		} else {
			mr.init();
			...
			writeToAdmin(request, response, "Room \"" + roomName + "\" has been opened.");
		}
	}
	...
	//create the room
		...
		if ((mr == null) || (mr.roomExists() )){
			...
			writeToAdmin(request, response, "Meeting room \"" + roomName + "\" already exists.  Choose a different room or \"" + ADMIN_COMMAND_OPEN_PARAM + "\" instead of \"" + ADMIN_COMMAND_CREATE_PARAM + "\".");
		} else {
			mr.init();
			if ((captioner = request.getParameter("capcode")) != null) {
				mr.setCaptioner(request, getProcessor(captioner));
			...
			meetingRooms.put(roomName, mr);
			writeToAdmin(request, response, "Room \"" + roomName + "\" has been created and is now open.");
		}
	}
	/**************************************************/
	public synchronized void processRequest(HttpServletRequest request, HttpServletResponse response) {
		...
		if (tmpStr != null) {
			...
			if (meetingRooms.containsKey(roomName)) {
				...
				if (adminCommand == null) {
					...
					meetingRooms.get(roomName).processRequest(request, response);
				} else if (meetingAdmin(request.getParameter(ADMIN_PASSWD_PARAM))) {
					//check for admin commands
					if (adminCommand.equals(ADMIN_COMMAND_CREATE_PARAM)) {
						writeToAdmin(request, response, "Room " + roomName + " has already been created and is open.");
					} else if (adminCommand.equals(ADMIN_COMMAND_OPEN_PARAM)) {
						writeToAdmin(request, response, "Room " + roomName + " is already open.");
					} else if (adminCommand.equals(ADMIN_COMMAND_CLOSE_PARAM)) {
						//close room
						meetingRooms.get(roomName).closeRoom();
						meetingRooms.remove(roomName);
						writeToAdmin(request, response, "Room " + roomName + " has been closed.");
						...
						writeToAdmin(request, response,  "Room " + roomName + " has been modified.");
					} else if (adminCommand.equals(ADMIN_COMMAND_CAPTION)) {
						meetingRooms.get(roomName).processCaptions(request, response);
					} else {
						writeToAdmin(request, response, "Administrative Command \"" + adminCommand + "\" not recognized.");
					}
				} else {
					//check for non-admin commands
					writeToAdmin(request, response, "Administrative password not valid for command \"" + adminCommand + "\".");
				}
			...
			//meeting room does not exist or not open
				...
				if (meetingAdmin(request.getParameter(ADMIN_PASSWD_PARAM))) {
					...
					if (adminCommand.equals(ADMIN_COMMAND_CREATE_PARAM)) {
						...
						writeToAdmin(request, response, "Meeting room \"" + roomName + "\" is not open or does not exist.");
					} else if (adminCommand.equals(ADMIN_COMMAND_CLOSE_PARAM)) {
						writeToAdmin(request, response, "Meeting room \"" + roomName + "\" is not open or does not exist.");
					} else {
						writeToAdmin(request, response, "Administrative Command \"" + adminCommand + "\" not recognized or room not open.");
					}
				} else {
					writeToAdmin(request, response, "Administrative password not valid for command \"" + adminCommand + "\".");
				}
			...
			writeToAdmin(request, response, "Please specify a meeting room.");
		}
		
		//Long doneTime = new Long(System.currentTimeMillis());
		if (debug) {
			long doneTime = System.nanoTime();
			long diff = (doneTime - startTime) / 1000;
	...
	/***************************************/
	private void writeToAdmin(HttpServletRequest request, HttpServletResponse response, String msg) {
